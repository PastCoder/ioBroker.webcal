...
import type { jsonEvent } from './calendarManager';
            ...
            if (this.nowFlag.allDay) {
                ...
                for (let i = 0; i < this.nowFlag.times.length; i++) {
                    ...
                    if (timeUntilStart <= 0 && timerUntilStop > 0) {
                        ...
                        stateText =
                            this.nowFlag.times[i].start != '00:00' ? `${i18n.from} ${this.nowFlag.times[i].start}` : '';
                        stateText +=
    ...
    syncIQontrolStates(): void {
        ...
        for (let i = 2; i < 7; i++) {
            iqontrolStates[i.toString()] =
                `${i18n[`weekDaysFull${(d + i) % 7}`]} ${i18n.inXDays.replace('%d', i.toString())}`;
        }
